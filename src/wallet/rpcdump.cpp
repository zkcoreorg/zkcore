...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid zkCore address or script");
    }
...
UniValue importwallet_impl(const UniValue& params, bool fHelp, bool fImportZKeys)
{
    ...
    while (file.good()) {
        ...
        // Let's see if the address is a valid zkCore spending key
        if (fImportZKeys) {
            ...
            if (IsValidSpendingKey(spendingkey)) {
                ...
                // Not a valid spending key, so carry on and see if it's a zkCore style t-address.
            }
        }
...
UniValue z_exportwallet(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename, saved in folder set by zkcored -exportdir option\n"
            "\nResult:\n"
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    if (exportdir.empty()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Cannot export wallet until the zkcored -exportdir option has been set");
    }
    ...
    // produce output
    file << strprintf("# Wallet dump created by zkCore %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
